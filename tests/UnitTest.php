...
class UnitTest extends TestCase
{
    ...
    public function test_client_is_instance_of_factory()
    {
        ...
        $this->assertSame(transmit('httpbin'), Http::scope('httpbin'));
    ...
    public function test_fake()
    {
        ...
        $response = Http::scope('localhost')->get('200');
        $this->assertEquals(200, $response->status());
        $this->assertTrue($response->ok());

        $response = Http::scope('localhost')->get('400');
        $this->assertEquals(400, $response->status());
        $this->assertTrue($response->clientError());

        $response = Http::scope('localhost')->get('500');
        $this->assertEquals(500, $response->status());
        $this->assertTrue($response->serverError());

        $response = Http::scope('localhost')->get('/foo');
        $this->assertEquals(404, $response->status());
    }

    public function test_request_uri()
    {
        $response = Http::scope('postman-echo')->get('400');
        $this->assertEquals(400, $response->status());
        $this->assertTrue($response->clientError());

        $response = Http::scope('postman-echo')->get('500');
        $this->assertEquals(500, $response->status());
        $this->assertTrue($response->serverError());

        $response = Http::scope('postman-echo')->get('/time/now');
        $this->assertEquals(200, $response->status());
        $this->assertTrue($response->ok());
